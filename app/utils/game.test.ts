...
import {getLastGuessBounds, verifyGuessAgainstTarget, validateGuess, isValidMove, getUpdatedColors, isValidBoard} from './game';

describe('getLastGuessBounds', () => {
  test('identifies new game boundary', () => {
    expect(getLastGuessBounds("????????????????????????????????????")).toStrictEqual({start: 0, end: 0});
  });

  test('identifies first play game boundary', () => {
    expect(getLastGuessBounds("111111??????????????????????????????")).toStrictEqual({start: 0, end: 6});
  });

  test('identifies last play game boundary', () => {
    expect(getLastGuessBounds("111111111111111111111111111111111111")).toStrictEqual({start: 30, end: 36});
  });

  test('identifies middle game boundary', () => {
    expect(getLastGuessBounds("111111111111111111??????????????????")).toStrictEqual({start: 12, end: 18});
  });
});
...
describe('getUpdatedColors', () => {
  test('all no-op', () => {
    ...
    const answer = "1/2+10"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
  });
  ...
  test('all incorrect guesses', () => {
    ...
    const oldColors = "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW";
    const answer = "1/2+10"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("RRRRRRWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
  });
  ...
  test('all misplaced guesses', () => {
    ...
    const oldColors = "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW";
    const answer = "1/2+10"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("YYYYYYWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
  });
  ...
  test('all correct guesses', () => {
    ...
    const oldColors = "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW";
    const answer = "1/2+10"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("GGGGGGWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
  });
  ...
  test('de-duplicates misplaced guesses with FIFO', () => {
    ...
    const oldColors = "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW";
    const answer = "7/32/2"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("RGRGRRWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
  });
  ...
  test('mix of incorrect, misplaced, and correct guesses', () => {
    ...
    const oldColors = "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW";
    const answer = "72/2/2"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("GYRYRRWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
  });
  ...
  test('complex case correctness in middle of board', () => {
    ...
    let answer = "72/2/2"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("WWWWWWWWWWWWWWWWGYRYRRWWWWWWWWWWWWWW")
  ...
  test('complex case correctness at end of board', () => {
    ...
    let oldColors = "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW";
    let answer = "72/2/2"
    expect(getUpdatedColors(newBoard, oldColors, answer)).toBe("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWGYRYRR")
...
describe('isValidMove', () => {
  test('corrupt board manipulation is rejected', () => {
    expect(isValidMove("?????????????????", "????????????????????????????????????", 10)).not.toBeUndefined();
    expect(isValidMove("?????????????????????????z??????????", "????????????????????????????????????", 10)).not.toBeUndefined();
    expect(isValidMove("??????????????111111????????????????", "????????????????????????????????????", 10)).not.toBeUndefined();
    expect(isValidMove("111111111111111?????????????????????", "111111111111????????????????????????", 10)).not.toBeUndefined();
  });
