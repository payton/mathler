 ...
 */
export function isValidMove(newBoard: string, oldBoard: string, target: number): string | undefined {
    ...
    if (start < 0 || end < 0 || start >= newBoard.length || end >= newBoard.length || (start > 0 && newBoard[start - 1] === "?")) {
        return "Invalid guess. You must fill in the entire row.";
    }
...
export function getUpdatedColors(newBoard: string, oldColors: string, answer: string): string {
    ...
    for (var i = 0; i < guess.length; i++) {
        if (guess[i] === answer[i]) {
            ...
            remainingLetters = remainingLetters.substring(0, idx) + remainingLetters.substring(idx + 1);
            color += "G";
    ...
    return oldColors.substring(0, bounds.start) + color + oldColors.substring(bounds.end);
}